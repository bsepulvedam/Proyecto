...
[IMPLEMENTADO 4B-3C/3D] ADMIN/JEFATURA lista y filtra trabajadores/período → abre calendario individual → revisa sesiones, incidencias y jornadas pagables → invoca acciones administrativas protegidas → consulta tarifa efectiva → exporta el mismo resumen/detalle a XLSX. El portal usa `ASISTENCIA_SUPERVISAR`, CSRF y servicios comunes; no expone coordenadas. [IMPLEMENTADO EN ÁRBOL 4B-3D] ADMIN abre tarifas → consulta historia global/individual → confirma monto y vigencia → crea una versión append-only con `ADMIN_ACCESS` y CSRF. Horas/días extra, PDF y detalle offline permanecen fuera de alcance.
