...
[IMPLEMENTADO EN ÁRBOL 4B-3C] ADMIN/JEFATURA lista y filtra trabajadores/período → abre calendario individual → revisa sesiones, incidencias y jornadas pagables → invoca las acciones administrativas protegidas → consulta tarifa efectiva. El portal usa `ASISTENCIA_SUPERVISAR`, CSRF y los servicios comunes; no expone coordenadas. [PENDIENTE 4B-3D+] exportación individual/conjunta a Excel, administración web de tarifas, horas/días extra y detalle de errores/offline.
