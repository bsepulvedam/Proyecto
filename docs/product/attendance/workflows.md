...
[IMPLEMENTADO 4B-1 backend] validar evidencia → evaluar todas las zonas activas configuradas → elegir la más cercana → persistir evidencia y evaluación → abrir incidencia por fuera de rango o baja precisión. [IMPLEMENTADO 4B-2 web] solicitar GPS únicamente al pulsar el botón → enviar evidencia con CSRF → resolver Worker autenticado → invocar el servicio 4B-1. Fallos GPS o falta de marcaje no se convierten automáticamente en ausencia.
