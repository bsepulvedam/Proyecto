...
def _return_from_form(form) -> DevolucionCreate:
    product_ids, quantities = form.get("producto_id", []), form.get("cantidad", [])
    if not product_ids:
        raise InventoryMovementError("Agrega al menos un producto al carrito.")
    lines = []
    for index, product_id in enumerate(product_ids):
        try:
            lines.append(LineaDevolucionCreate(
                producto_id=int(product_id), cantidad_presentaciones=Decimal(quantities[index].replace(",", ".")),
            ))
        except (ValueError, InvalidOperation, IndexError, ValidationError) as exc:
            raise InventoryMovementError(f"Revisa la cantidad de la línea {index + 1}.") from exc
...
@router.get("/devolucion", response_class=HTMLResponse, name="inventory_return")
def return_form(request: Request, db: Session = Depends(get_db)):
    return templates.TemplateResponse(request=request, name="inventory/return.html", context=context(
        empresas=listar_empresas(db), fecha=date.today(), error=None,
        active_page="inventory_return", page_title="Devolución de productos",
    ))


@router.post("/devolucion", response_class=HTMLResponse, name="create_inventory_return")
async def return_create(request: Request, db: Session = Depends(get_db)):
    try:
        movement = create_return(db, _return_from_form(await _form(request)))
    except (InventoryMovementError, ValidationError) as exc:
        return templates.TemplateResponse(request=request, name="inventory/return.html", context=context(
            empresas=listar_empresas(db), fecha=date.today(), error=str(exc),
            active_page="inventory_return", page_title="Devolución de productos",
        ...
        return templates.TemplateResponse(request=request, name="inventory/return.html", context=context(
            empresas=listar_empresas(db), fecha=date.today(),
            error="No fue posible registrar la devolución. No se guardó ninguna línea.",
            active_page="inventory_return", page_title="Devolución de productos",
        ), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
