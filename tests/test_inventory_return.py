...
from app.schemas.movimiento_inventario import DevolucionCreate, LineaDevolucionCreate, LineaRecepcionCreate, RecepcionCreate
from app.services.inventario_movimiento_service import (
    InventoryMovementError, calculate_stock_from_movements, create_receipt, create_return,
)


class InventoryReturnTests(unittest.TestCase):
    def setUp(self):
    ...
    def do_return(self, lines, empresa_id=None):
        return create_return(self.db, DevolucionCreate(
            empresa_id=empresa_id or self.bol_id, fecha=date(2026, 8, 29), lineas=lines,
        ))

    def line(self, product, quantity):
        return LineaDevolucionCreate(producto_id=product.id, cantidad_presentaciones=quantity)

    def test_return_increases_stock_and_gets_sequential_document_number(self):
        self.receive(self.bol1, 10)
        movement = self.do_return([self.line(self.bol1, 3)])
        self.assertEqual(movement.tipo, "DEVOLUCION")
        self.assertEqual(movement.numero_documento, "MOV-000002")
        stock = calculate_stock_from_movements(self.db)
        self.assertEqual(stock[(self.bol_id, self.bol1.id)], Decimal("13.000"))

    def test_return_without_any_prior_receipt_is_allowed(self):
        movement = self.do_return([self.line(self.bol1, 2)])
        self.assertEqual(movement.tipo, "DEVOLUCION")
        stock = calculate_stock_from_movements(self.db)
        self.assertEqual(stock[(self.bol_id, self.bol1.id)], Decimal("2.000"))

    def test_return_partial_line_failure_rolls_back_whole_movement(self):
        self.receive(self.bol1, 10)
        with self.assertRaises(InventoryMovementError):
            self.do_return([self.line(self.bol1, 2), self.line(self.alm1, 1)])
        stock = calculate_stock_from_movements(self.db)
        self.assertEqual(stock[(self.bol_id, self.bol1.id)], Decimal("10.000"))
        self.assertEqual(self.db.scalar(select(func.count()).select_from(MovimientoInventario).where(MovimientoInventario.tipo == "DEVOLUCION")), 0)

    def test_return_cross_company_product_is_rejected(self):
        self.receive(self.alm1, 10)
        with self.assertRaises(InventoryMovementError):
            self.do_return([self.line(self.alm1, 1)], empresa_id=self.bol_id)

    def test_return_decimal_quantity_rejected_when_unit_forbids_it(self):
        with self.assertRaises(InventoryMovementError):
            self.do_return([self.line(self.bol8, Decimal("2.5"))])

    def test_return_empty_cart_is_rejected(self):
        data = DevolucionCreate.model_construct(empresa_id=self.bol_id, fecha=date.today(), lineas=[])
        with self.assertRaises(InventoryMovementError):
            create_return(self.db, data)

    def test_return_without_origen_bot_defaults_to_erp_web(self):
        movement = self.do_return([self.line(self.bol1, 1)])
        self.assertEqual(movement.origen, "ERP_WEB")
        self.assertIsNone(movement.actor_referencia)

    def test_return_accepts_bot_origin_and_actor(self):
        origen_bot = OrigenMovimientoBot(origen="BOT_TELEGRAM", actor_referencia="Juan Pérez (Telegram id 5839201)")
        movement = create_return(self.db, DevolucionCreate(
            empresa_id=self.bol_id, fecha=date(2026, 8, 29), lineas=[self.line(self.bol1, 1)],
        ), origen_bot)
        self.assertEqual(movement.origen, "BOT_TELEGRAM")
        self.assertEqual(movement.actor_referencia, "Juan Pérez (Telegram id 5839201)")

    def test_return_carries_guide_and_reference_fields(self):
        movement = create_return(self.db, DevolucionCreate(
            empresa_id=self.bol_id, fecha=date(2026, 8, 29), guia_despacho="GD-0001",
            referencia="MOV-000001", lineas=[self.line(self.bol1, 1)],
        ))
        self.assertEqual(movement.guia_despacho, "GD-0001")
        self.assertEqual(movement.referencia, "MOV-000001")
